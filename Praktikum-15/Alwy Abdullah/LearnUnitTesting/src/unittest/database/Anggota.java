...
public class Anggota {
    ...
    public ArrayList<Anggota> getByNamaAlamatAndTelpon(String nama, String alamat, String telpon) {
        ArrayList<Anggota> ListAnggota = new ArrayList();
        ResultSet rs;

        if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telpon.trim().length() > 0)) {
            rs = DBHelper.selectQuery("SELECT * FROM anggota Where nama = '"
                    + nama + "' and alamat = '" + alamat + "' and telpon = '" + telpon + "'");
        } else if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telpon.trim().length() == 0)) {
            rs = DBHelper.selectQuery("SELECT * FROM anggota Where nama = '"
                    + nama + "' and alamat = '" + alamat + "'");
        } else if ((nama.trim().length() > 0) && (alamat.trim().length() == 0) && (telpon.trim().length() > 0)) {
            rs = DBHelper.selectQuery("SELECT * FROM anggota Where nama = '"
                    + nama + "' and telpon = '" + telpon + "'");
        } else if ((nama.trim().length() == 0) && (alamat.trim().length() > 0) && (telpon.trim().length() > 0)) {
            rs = DBHelper.selectQuery("SELECT * FROM anggota Where alamat = '"
                    + alamat + "' and telpon = '" + telpon + "'");
        } else if ((nama.trim().length() > 0) && (alamat.trim().length() == 0) && (telpon.trim().length() == 0)) {
            rs = DBHelper.selectQuery("SELECT * FROM anggota Where nama = '"
                    + nama + "'");
        } else if ((nama.trim().length() == 0) && (alamat.trim().length() > 0) && (telpon.trim().length() == 0)) {
            ...
            rs = DBHelper.selectQuery("SELECT * FROM anggota WHERE telpon "
                    + " = '"+telpon+"'");
        }
